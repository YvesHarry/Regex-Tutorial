# Matching an Email Regex: Understanding the Components by Yves Harry Deslouches
...
# Summary
Throughout this tutorial, the featured regular expression (regex) as seen below, will be referenced for analysis in regards to each regex component to ensure new-comers and academics alike are able to understand and grasp material with absolute clarity. The regex components discussed in this document include: Anchors, Quantifiers, Grouping Constructs, Bracket Expressions, Character Classes, and lastly Character Escapes.
...
Our **regex featured in this tutorial:** `/^([a-z0-9_\.-]+)@([\da-z\.-]+)\.([a-z\.]{2,6})$/` contains (3) Grouping Constructs enclosed among the parentheses `()`.
...
**Example:** Consider the following email address: `yves_1234-@bootcamp.edu`

The local part, of the username, is `yves_1234-`.

Now let's break it down according to the bracket expression `[a-z0-9_\.-]`:
- `yves` contains lowercase letters `t`, `h`, `o`, `m`, and `s`, matching the `a-z` part of the expression.
...
In the example, the bracket expression `[a-z0-9_\.-]` successfully matches the local part `yves_1234-` of the email address.
...
**Example:** Consider the following email address: `yves1234@boot-camp2023.edu`
...
**Example:** Consider the following email address: `yves_1234.email@bootcamp123.examp.le`
...
1. `yves.des@bootcamp.edu` **(valid)**
2. `yves@indes@bootcamp.edu` **(invalid)**
...
**Example:** Consider the following two email addresses:
1. `yves-des@bootcamp.com` **(valid)**
2. `yves--des@bootcamp.com` **(valid)**
3. `yves@des_bootcamp.com` **(invalid)**
...
**Example:** Consider the following two email addresses:
1. `yves.harry2023@bootcamp.com` **(valid)**
2. `yves123_val@bootcamp.com` **(valid)**
3. `ydes@bootcamp7.com` **(valid)**
4. `yves.des@2023bootcamp.com` **(invalid)**
...
Follow me on Github at [Yves Harry Deslouches](https://github.com/YvesHarry). Additional questions or concerns? feel free to contact me via emial yhdeslouches@hotmail.com


Deployed GitHub-Gist Link:[Deployed GitHub-Gist Link: Click Here](https://gist.github.com/YvesHarry/0df34d55002b03afb602eb3aae97f694)

GitHub Repository:[GitHub Repository: Click Here](https://github.com/YvesHarry/Regex-in-JavaScript-A-Comprehensive-Tutorial-by-Yves-Harry-Deslouches)
